...
const makeDistribution = (scale, series) => {
  ...
  const domain = distList.reduce(({ min, max }, [, replyCount]) => ({
    min: Math.min(replyCount, min),
    max: Math.max(replyCount, max),
  }), { min: Infinity, max: -Infinity });

  const numberScale = d3Scale.scaleLinear([0, domain.max], [SPARK_LINE_HEIGHT + SPARK_LINE_PADD, SPARK_LINE_PADD]);
...
const useSpark = (replySeries, sparkTimeScale) => useMemo(
  () => {
    if (!replySeries) {
      return '';
    }
