...
export default class BaseModel extends objection.Model {
  ...
  static query(trx) {
    const query = super
      .query(trx)
      .onError(({ constructor, data: { error } = {} }) => {
        ...
        switch (constructor) {
          ...
          case objection.CheckViolationError:
            throw new BadRequestError(
              error || errors.GLOBAL_ERR_CHECK_VIOLATION,
            );
          case objection.ForeignKeyViolationError:
            throw new ConflictError(
              error || errors.GLOBAL_ERR_FOREIGN_VIOLATION,
            );
          ...
          case objection.UniqueViolationError:
            throw new ConflictError(
              error || errors.GLOBAL_ERR_UNIQUE_VIOLATION,
            );
